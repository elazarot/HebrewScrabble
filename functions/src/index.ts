...
export const onMatchUpdate = onDocumentUpdated('matches/{matchId}', async (event) => {
  ...
  if (guestJoined) {
    ...
    if (targetUid) {
      const actionText = afterData.lastMove ? (
        afterData.lastMove.action === 'PLAY' ? `שיחק מילה: "${afterData.lastMove.word}"` :
        afterData.lastMove.action === 'PASS' ? 'עבר תור' : 'החליף אריחים'
      ) : 'שיחק מהלך';
 ...
 */
async function sendPush(uid: string, title: string, body: string, matchId: string) {
  try {
